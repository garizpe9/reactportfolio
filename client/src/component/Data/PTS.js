...
egen duptot = total(dup)
	...
	if duptot == 0 {
		disp "No Duplicates Found"
			...
			export excel dup *** if dup>1 using City__Analysis.xlsx, firstrow(varlabels) sheet(Duplicates) 
			}
			else if fileexists("City__Analysis.xlsx") == 1 {
			export excel dup *** if dup>1 using City__Analysis.xlsx, firstrow(varlabels) sheet(Duplicates, replace)  
		}
}
...
rename Pretrial_Service_Binary13 PTS13
	...
	/*c. Create Binary of receiving pretrial service*/
gen ReceivedPTS =. 
